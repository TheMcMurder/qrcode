...
fn render_finder_module(x: usize, y: usize, module_size: usize, shape: &FinderShape) -> String {
    ...
    for (y, row) in matrix.iter().enumerate() {
        for (x, &is_dark) in row.iter().enumerate() {
            if is_dark {
                ...
                if is_finder {
                    svg.push_str(&render_finder_module(x, y, module_size, &config.finder_shape));
                } else {
